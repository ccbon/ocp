...
static int btree_node_get(btree_node_t *nodep, void *value_key, void **valuepp, btree_config_t *cfg) {
	...
	if (cmp == 0) {
		*valuepp = nodep->value;
		...
		btree_node_get(nodep->left, value_key, valuepp, cfg);
	} else {
		btree_node_get(nodep->right, value_key, valuepp, cfg);
	}
	return 0;
}
...
int btree_ordered_list(btree_t *btreep, linked_list_t **listpp) {
	if (listpp == NULL) {
		return 1;
	}
	*listpp = ll_create();
	return btree_node_ordered_list(btreep->root, *listpp);
}
