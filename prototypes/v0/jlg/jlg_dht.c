...
dht_agent_t *dht_get_agent_from_key(dht_t *dhtp, char *key) {
	return dht_default_get_agent_from_key(dhtp, key);
}

// set a pair key value. value is duplicated
int dht_set(dht_t *dhtp, char *key, char *valuep) {
	return dht_default_backup_set(dhtp, key, valuep);
}

// get a value without modifying the structure
int dht_get(dht_t *dhtp, char *key, char **valuepp) {
	return dht_default_backup_get(dhtp, key, valuepp);
}

int dht_remove(dht_t *dhtp, char *key) {
	return dht_default_backup_remove(dhtp, key);
}
...
btree_t *dht_get_network(dht_t *dhtp) {
...
int dht_nice_detach(dht_t *dhtp) {
	return dht_default_nice_detach(dhtp);
}
